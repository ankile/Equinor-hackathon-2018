...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if (not drag_back_point) and speed > 0.25 and (speed**2.0)*0.095 - 0.2 > distance((pos.x, pos.y), waypoint) and distance((pos.x, pos.y), waypoint) > 1.5:
        ...
        if drag_back_point and speed < 0.24:
            path = path[1:]
            (x, y) = path[0]
            drone.set_target(x, y, 0)
            print("Target set to ", x, y)
